    ...
    for c in range(1,k+1):  # c : capacity
        if weight > c:
            ...
            dp[i][c] = max(dp[i-1][c], dp[i-1][c-weight]+value)
                          # 여기서 비교하고 있는 dp[i-1][c] 와 dp[i-1][c-weight] 모두 이전 아이템까지의 값들이다.
                          # 지금 아이템을 넣을지 말지를 결정하는 상황이므로,
                          # 이전 아이템까지의 결과들을 가지고 비교하는 것이다.
                          # -> 현재 아이템을 넣지 않는다는 것 = 전체 무게가 안변한다는 것 = 이전 아이템까지에서의 같은 무게의 value를 가져옴.
                          # -> 현재 아이템을 넣는 다는 것 = 현재 전체 무게c 안에서 넣어야 하므로, c-weight의 상황에서 현재의 아이템을 넣는 다는 것
                          #                          =  이전 아이템까지에서의 c-weight무게에 대한 value를 가져온 후 현재 아이템 value를 더해줌.
                          # 이렇게 얻어진 두개의 value를 비교해서 큰 값으로 갱신한다.
print(dp[-1][k])
