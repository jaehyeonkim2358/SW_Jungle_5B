...
# 일차원 배열을 활용.
# 어차피 모든 경우의 수가 누적 되는 상황이므로, 현재 인덱스에 가지고 있는 값에 추가로 계산된 경우의 수를 더해주면서 진행하면 됨.
...
# 위에서처럼 일차원 배열을 사용하는 것이 시간/메모리 절약 부분에서 좋다.
# 아래는 과정을 이해하기 쉽도록 이차원 배열을 사용.
#
# for _ in range(int(input())):
#     n = int(input())
...
#             if x < coin:
#                 d[i+1][x] = d[i][x]                   # 현재 동전보다 작은 인덱스들 이전 동전의 값으로 덮어씌움.
#             else:
#                 d[i+1][x] = d[i][x] + d[i+1][x-coin]  # 현재동전의 x원에 대한 경우의 수를,
#                                                       # 이전 동전까지의 x원에 대한 경우의 수 + 현재 동전의 x-coin원에 대한 경우의수로 갱신.
